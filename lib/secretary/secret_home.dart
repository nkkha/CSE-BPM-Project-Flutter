...
class RequestList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for (RequestNVQS requestNVQS in requestList) {
      if (requestNVQS.status.contains("TK")) {
      ...
      children: <Widget>[
        ListView.builder(
          ...
          itemBuilder: (context, index) {
            return ListTile(
              title: Text(
                  'Request ID: ${newRequestList[index].id}, User ID: ${newRequestList[index].userID}'),
              subtitle: Text(
                  'Content: ${newRequestList[index].content}, Status: ${newRequestList[index].status}'),
            );
          },
