...
//  WebService._internal();
  ...
  // StepInstance
  Future<List<StepInstance>> getListStepInstance(
  ...
  Future<void> patchRequestInstanceFailed(int requestInstanceID, Function updateData) async {
    var resBody = {};
  ...
  Future<void> patchRequestInstanceStep(int requestInstanceID, Function update) async {
    var resBody = {};
    // indexType = 1: Reject, indexType = 2: Approve
    String str = json.encode(resBody);
  ...
  Future<void> patchRequestInstanceFinished(int requestInstanceID, Function update) async {
    var resBody = {};
    // indexType = 1: Reject, indexType = 2: Approve
    resBody["Status"] = "done";
    String str = json.encode(resBody);
  ...
  Future<void> getNextStep(RequestInstance requestInstance, int nextStepIndex, Function update) async {
    final response = await http.get('http://nkkha.somee.com/odata/tbStep?\$filter=RequestID eq ${requestInstance.requestID} and StepIndex eq $nextStepIndex');
    ...
    if (response.statusCode == 200) {
      ...
      if (nextStepSize > 0) {
        ...
        patchRequestInstanceFinished(requestInstance.id, update);
      }
    } else {
      throw Exception('Failed to load');
    }
  ...
  Future<void> postCreateNextStep(int requestInstanceID, int stepID, Function update) async {
